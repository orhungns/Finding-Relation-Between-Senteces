...
from keras.layers import Concatenate ,merge, recurrent, Dense, Input, Dropout, TimeDistributed
...
def yield_examples(fn, skip_no_majority=True, limit=None):
  for i, line in enumerate(open(fn, encoding='utf-8-sig')):
...
def get_data(fn, limit=None):
  ...
  return left, right, Y
def get_data_test(fn, limit=None):
...
def yield_examples_test(fn, skip_no_majority=True, limit=None):
  for i, line in enumerate(open(fn, encoding='utf-8-sig')):
    ...
    data = json.loads(line)
    label = data['gold_label']
    s1 = ' '.join(extract_tokens_from_binary_parse(data['sentence2_binary_parse']))
    s2 = ' '.join(extract_tokens_from_binary_parse(data['sentence1_binary_parse']))
    if skip_no_majority and label == '-':
      continue
    yield (label, s1, s2)

training = get_data('train.jsonl')
validation = get_data('dev.jsonl')
test = get_data_test('test.jsonl')
...
joint = Concatenate(axis=1)([prem, hypo])
...
sentence1='Bir erkek yolda yürüyor'
sentence2='Bir kadın yüzüyor'
test_sentence_pairs = ([sentence1],[sentence2])

test_sentence_pairs = prepare_data2(test_sentence_pairs)
